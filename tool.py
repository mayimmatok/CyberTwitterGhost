...
def _download_one_snapshot(item: dict, user_dir: Path):
    ...
    urls = [
        f"https://web.archive.org/web/{item['timestamp']}if_/{item['original']}",
        f"https://web.archive.org/web/{item['timestamp']}/{item['original']}",
    ]
    ...
    for attempt in range(1, DOWNLOAD_MAX_ATTEMPTS + 1):
        ...
        # 指数退避 + 抖动，降低同站点并发抖动导致的瞬时失败。
        backoff = min(8.0, 0.5 * (2 ** (attempt - 1))) + random.uniform(0.0, 0.6)
        time.sleep(backoff)

    if not errors:
        return item["status_id"], False, file_path.name, "download_failed"

    # 保留简短错误摘要，便于 checkpoint 中定位失败类型。
    uniq = []
    for e in errors:
        if e not in uniq:
            uniq.append(e)
    return item["status_id"], False, file_path.name, "|".join(uniq[:3])
...
def parallel_download_snapshots(items, user_dir: Path, checkpoint: dict, progress: ProgressBars):
    ...
    base_workers = min(12, max(4, os.cpu_count() or 4))
    item_by_sid = {it["status_id"]: it for it in pending}
    unresolved = list(pending)
    resolved_count = 0
    success_count = 0
    fail_count = 0

    for round_idx in range(1, DOWNLOAD_RETRY_ROUNDS + 1):
        if not unresolved:
            break

        round_workers = max(2, base_workers // (2 ** (round_idx - 1)))
        next_unresolved = []
        ...
        with ThreadPoolExecutor(max_workers=round_workers) as executor:
            ...
            for future in as_completed(future_map):
                ...
                if ok:
                    ...
                    if round_idx < DOWNLOAD_RETRY_ROUNDS:
                        next_unresolved.append(item_by_sid[sid])
                    else:
        ...
        if unresolved and round_idx < DOWNLOAD_RETRY_ROUNDS:
            # 轮次间短暂退避，降低同一批失败请求立刻重打导致的连续失败。
            time.sleep(1.0 + 0.7 * round_idx)

    progress.finish_step(f"新增下载成功 {success_count} 条，失败 {fail_count} 条")


def retry_failed_downloads_once(items, user_dir: Path, checkpoint: dict, progress: ProgressBars):
    failed = checkpoint["failed_downloads"]
    processed = checkpoint["processed"]
    item_map = {item["status_id"]: item for item in items}

    retry_items = []
    for sid in list(failed.keys()):
        item = item_map.get(sid)
        if item is not None:
            retry_items.append(item)

    progress.start_step("重试首次下载失败快照", max(len(retry_items), 1))
    if not retry_items:
        progress.finish_step("无失败快照，跳过重试")
        return 0
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_map):
            sid, ok, filename, err = future.result()
            if ok:
                checkpoint["files"][sid] = filename
                failed.pop(sid, None)
                # 首次解析时该条可能被记为 None，重试成功后需要允许重新解析。
                processed.pop(sid, None)
                success_count += 1
            else:
                failed[sid] = err or "retry_failed"
            progress.advance(1)

    progress.finish_step(f"重试成功 {success_count} 条，仍失败 {len(retry_items) - success_count} 条")
...
def export_empty_human_records(username: str, records):
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    out_file = DATA_DIR / f"{username}_empty_human_for_manual_completion.json"
    out_file.write_text(json.dumps(records, ensure_ascii=False, indent=2), encoding="utf-8")
    return out_file
...
def collect_for_user(username: str, fill_empty_human: bool = False):
    ...
    total_steps = 7 if fill_empty_human else 6
    ...
    retried_success = retry_failed_downloads_once(items, user_dir, checkpoint, progress)
    ...
    if fill_empty_human:
        ...
        empty_human_records = [
            r for r in records
            if not ((r.get("conversations") or [{}])[0].get("value") or "").strip()
        ]
        if empty_human_records:
            manual_file = export_empty_human_records(username, empty_human_records)
            print(f"[{username}] 空human样本已单独导出: {manual_file} ({len(empty_human_records)} 条)")
...
def verify_torch_runtime(python_bin: Path, require_gpu: bool):
    ...
    if require_gpu and not info.get("cuda_available"):
        raise RuntimeError(
...
def main():
    ...
    for username in usernames:
        try:
            ...
            if not fill_empty_human:
                empty_file = DATA_DIR / f"{username}_empty_human_for_manual_completion.json"
                if empty_file.exists():
                    empty_human_files.append(empty_file)
        except Exception as exc:
    ...
    if empty_human_files:
        merged_empty_path, merged_empty_count = merge_empty_human_files(
            empty_human_files,
            DATA_DIR / "all_users_empty_human_for_manual_completion.json",
        )
        print(f"空human汇总: {merged_empty_path} ({merged_empty_count} 条)")
